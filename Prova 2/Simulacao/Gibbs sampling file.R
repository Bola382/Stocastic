...
gibbs = function(folder,repl,dados,G,phi=.3,Q,burn,thin){
 ...
 prob.samp = tau2.samp = Delta.samp = matrix(NA, nrow = 2, ncol = G)
 ...
 for(i in 2:Q){
  ...
  for(j in 1:G){
   ...
   Delta.samp[2,j] = rnorm(1, omega^2*S1/denom, omega*sqrt(tau2.samp[2,j]/denom))
  }
